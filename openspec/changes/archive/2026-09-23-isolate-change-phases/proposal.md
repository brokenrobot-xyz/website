...
- **Each reviewing agent writes its own report file.** `frontend-qa-engineer` writes the Verify
  ...
  Each gains the `Write` tool and is held to that one path by instruction, the way its unrestricted
  `Bash` grant already is; the diff at the gate is the check. A report then never passes through the
  main thread. Every other write stays forbidden.
...
- **Commits run in a general-purpose subagent** that invokes the `committing-conventionally` skill,
  ...
  three questions up front: the branch, the touched-files list, and explicit staging. A change makes
  two commits, each after a human stop: the code commit after the implementation gate, then Archive,
  then the human looks at the merged specs, then the specs commit.
...
- **The intent graduates, then the docs follow the process.** `docs/development-workflow.md` gains
  the two review steps and the principle behind every hand-off, stated without naming a tool: a phase
  starts from the files the phase before it left and ends with a file, and the session that runs a
  change routes between phases rather than doing their work. `docs/tooling/workflow.md` takes the
  process table — each step's owner, input, and output on disk — in place of its phase table, drops
  the line that says planning has no agent by design, and its schema-reconcile recipe gains a step,
  because the fork no longer matches upstream's artifact list. The known-gaps entry is deleted last,
  once the intent has both homes and both of its exits are reached.
...
- **Not** running this change through the process it creates. This change is planned and applied
  under today's flow; the new flow is exercised on the change after it. Its `brief.md` and
  `review.md` are written by hand, which is a record, not a run.
...
- `.claude/agents/frontend-qa-engineer.md` — the Verify report file, the preflight gate, ticking its
  own Verify items, the `Write` grant for the report, and the `Edit` grant for the ticks.
- `.claude/agents/frontend-code-reviewer.md` — the findings file and the same grant.
...
**This change's folder**

- `openspec/changes/isolate-change-phases/brief.md` and `review.md`, written by hand at planning
  time, so the change satisfies the gate it introduces.

**Risk to check first**

- Adding a required artifact changes what OpenSpec expects of every change that already exists. The
  six archived changes carry no `brief.md` and no `review.md`. Whether
  `openspec validate --all --strict` and `openspec validate --archived` still pass is
  **unverified**, and `npm run specs:check` gates CI on both. The first task establishes this against
  a throwaway copy of the schema, before any schema edit.
