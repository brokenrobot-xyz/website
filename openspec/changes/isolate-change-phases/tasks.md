...
- [x] 1.1 Copy `openspec/schemas/` to a throwaway location outside the repository, add a `brief`
...
- [x] 1.2 Run `openspec validate --all --strict` and `openspec validate --archived` inside the copy,
      ...
      _Recorded 2026-09-22, OpenSpec 1.13.1, copy in the session scratchpad:_
      `openspec validate --all --strict` → exit 0, `Totals: 6 passed, 0 failed` (the copied active
      change passed with `brief.md` and `review.md` absent);
      `openspec validate --archived` → exit 0, `Totals: 1 passed, 0 failed`;
      `openspec instructions apply` → `state: "blocked"`, `missingArtifacts: ["review"]`,
      `Cannot apply this change yet. Missing artifacts: review.` — the expected not-ready.

## 2. Declare the two artifacts in the schema

- [x] 2.1 Add the `brief` artifact to `openspec/schemas/frontend-change/schema.yaml` — first in the
...
- [x] 2.2 Add `openspec/schemas/frontend-change/templates/brief.md` with the five headings from
      ...
      `# Brief`.
- [x] 2.3 Add the `review` artifact, requiring `tasks`, and add `review` to `apply.requires`. Verify
...
- [x] 2.4 Add `openspec/schemas/frontend-change/templates/review.md`, opening with a `# Review`
...
- [x] 2.5 Run `npm run specs:check` and verify it passes with the archived changes untouched.

## 3. The two new agents

- [x] 3.1 Write `.claude/agents/frontend-planner.md`: opus, wraps the vendored propose and update
...
- [x] 3.2 Write `.claude/agents/frontend-proposal-reviewer.md`: opus, granted `Write` and held by
...
- [x] 4.1 Update `.claude/agents/frontend-qa-engineer.md`: it owns all of Verify including the
...
- [x] 4.2 Update `.claude/agents/frontend-code-reviewer.md`: it writes its findings to
...
- [x] 5.1 Write the `coordinating-changes` skill under `.claude/skills/`, with two sections — start a
...
- [x] 5.2 Add one pointer line to `CLAUDE.md` naming the coordinator skill as the entry point for a
      change. Verify the line names the skill and adds no copy of the sequence.

## 6. Configuration and documentation

- [x] 6.1 Update the apply guidance in `openspec/config.yaml` so Verify is handed to
...
- [x] 6.2 Update `docs/tooling/workflow.md`: the phase table is replaced by the process table from
...
- [x] 6.3 Update `docs/development-workflow.md` to carry the two review steps in the phase list and
      ...
      appears in the page.
- [x] 6.4 After 6.2 and 6.3, delete the **The main session does the work it is meant to coordinate**
