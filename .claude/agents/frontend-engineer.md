...
- Run `npm run type:check`, `npm run lint:check`, `npm run format:fix` as you go to keep the tree green. Use the `preflight-checks` skill for the full gate — the `frontend-qa-engineer` re-runs it at Verify as the authoritative pass.
- **Stop at the Verify section.** Visual + a11y snapshot work belongs to the `frontend-qa-engineer` (via the `visual-regression-tests` skill). Report what you implemented and what remains to verify.
