...
- When the caller asks for the change's formatting to be fixed, run `npx prettier --write` with the paths from the change's file set. When the caller asks only for the gate, report the `format:check` failure and leave every file alone, because a gate that edits unasked hands back a working tree the caller never reviewed. Do not run `npm run format:fix` in either case, because that script rewrites the full repo glob and reformats untouched files the caller never asked you to touch.
...
- **type:check** — zero errors. `astro/tsconfigs/strictest` plus `strictNullChecks`: no implicit `any`, strict null checks. Explicit `any` and loose truthiness are ESLint rules, so they fail under `lint:check` instead.
...
- **format:check** — clean. When a failing file is outside the change's file set, report that file as pre-existing instead of reformatting it; the path-scoped `npx prettier --write` is only for the change's own files, and only when the caller asked for a fix.
...
- **build** — succeeds, **and** the built output requests nothing from a third party (the **Ad-free and tracking-free** and **Strict security** principles in `docs/vision.md` § Enduring principles, and the self-hosted fonts in `docs/tech-stack.md`). The site serves `default-src 'none'` with every fetch directive at `'self'` or `'none'`, so a third-party resource that reaches `dist/` is one the browser blocks — a silently broken page in production, not a style preference. This grep catches a third-party resource request at build time; expect empty output:

    ```bash
    grep -RoE '(src|srcset|poster)="https?://[^"]*"|<link [^>]*href="https?://[^"]*"|url\([^)]*https?://[^)]*\)|(fetch|import)\([^)]*https?://[^)]*\)' dist/ | grep -v 'brokenrobot\.xyz'
    ```

    The pattern matches resource-fetching forms only, so an outbound `<a href>` in prose, a same-origin canonical `<link>`, a relative `url(/img/…)`, and a `data:` URI all leave the pattern silent. The `url()` and `fetch()`/`import()` branches match on the parentheses rather than on a quote character, because `.prettierrc` sets `singleQuote`, an `is:inline` script body reaches `dist/` verbatim, and a branch anchored to `"` would miss every single-quoted call the repo's own formatter produces. Anything the grep prints is a third-party resource request — report that resource as a **build** failure with the offending file, even though `astro build` exited 0.

    When `astro build` exits 0, run the grep. A failed build leaves `dist/` emptied or half-written, so an empty result over that directory says nothing about the change — report the guardrail check as `not verified` next to the build failure, because empty output is this check's success signal and a failed build would otherwise produce its strongest claim from its weakest evidence.
...
Summarize each step as pass/fail.

- Take each step's verdict from that step's own exit status.
- For a failing step, quote the error printed first in that step's output as file:line + message.
- When a failing step reports its own error count, add that count, because the count gives the caller the size of the problem without this report reproducing the whole log.
- Do not rank the errors by which look genuine, because ranking substitutes your guess for the compiler's ordering and the error you judge most genuine is usually a cascade of the first.
- Report a step that never ran as `not run` rather than as pass, because a fabricated green line sends the caller to review with an error the gate claimed to have checked.
- When every step passes, say so plainly.
...
format:check   FAIL — docs/old-notes.md — 1 pre-existing failure in a file the change never touched; flagged, not fixed
