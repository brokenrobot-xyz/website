...
- **Theme selection on `<html>`** via the `data-theme` attribute. A tiny **inline script** in
  ...
  Astro emits a SHA-256 hash for it into the page's policy — not because inline scripts are
  allowed; they are not (see [Content-Security-Policy](#content-security-policy)). It uses no
  inline `on*` handlers, which the policy blocks outright.
...
1. **A `<meta>` policy in every page**, generated by `security.csp` in `astro.config.ts`. This is
   the strict layer: `script-src`/`style-src` carry per-build SHA-256 hashes and **no
   `unsafe-inline`**, so an injected inline script does not run. Because the hashes change every
   build, only the build can produce this layer — and because it lives in the HTML, it travels
...
- **A `<meta>` policy only governs what the parser reaches after it.** Anything earlier in
  `<head>` is covered by the header alone. Closing that window would require emitting the union
  of all page hashes into the edge config at build time; we have not done that.
